...
describe("auto schedules — auth split", () => {
  ...
  it("COOKIE POST /api/auto/schedules without a session → 401 (no schedule created)", async () => {
    requireUserMock.mockRejectedValue(new FakeUnauthorizedError("Sign in is required."));
    const { POST } = await import("@/app/api/auto/schedules/route");
    const req = new Request("https://forge.example/api/auto/schedules", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ kitRef: { source: "local", localKitId: "k" }, cron: "0 9 * * *", prompt: "go", budgetCents: 50, approvalId: "appr-x" })
    });
...
describe("auto schedules — create validation (cookie path)", () => {
  beforeEach(() => {
    requireUserMock.mockReset();
    resetStorage();
  ...
  async function post(body: Record<string, unknown>) {
    const { POST } = await import("@/app/api/auto/schedules/route");
    return POST(
      new Request("https://forge.example/api/auto/schedules", {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  it("bad cron → 400 invalid_request (no schedule created)", async () => {
    seedApproval();
    const res = await post({ kitRef: { source: "local", localKitId: "k" }, cron: "not a cron", prompt: "go", budgetCents: 50, approvalId: "appr-x" });
    expect(res.status).toBe(400);
    expect(((await res.json()) as { error: string }).error).toBe("invalid_request");
    expect(storageRef.current.state.schedules).toHaveLength(0);
  });

  it("missing approval → 403 approval_denied", async () => {
    const res = await post({ kitRef: { source: "local", localKitId: "k" }, cron: "0 9 * * *", prompt: "go", budgetCents: 50, approvalId: "appr-x" });
    expect(res.status).toBe(403);
    expect(((await res.json()) as { error: string }).error).toBe("approval_denied");
  });

  it("approvalId mismatch → 400 invalid_request", async () => {
    seedApproval();
    const res = await post({ kitRef: { source: "local", localKitId: "k" }, cron: "0 9 * * *", prompt: "go", budgetCents: 50, approvalId: "wrong-id" });
    expect(res.status).toBe(400);
  });

  it("budget over the approval ceiling → 403", async () => {
    seedApproval(50);
    const res = await post({ kitRef: { source: "local", localKitId: "k" }, cron: "0 9 * * *", prompt: "go", budgetCents: 100, approvalId: "appr-x" });
    expect(res.status).toBe(403);
  });

  it("valid → 201 and a future nextRunAt is computed", async () => {
    seedApproval();
    const res = await post({ kitRef: { source: "local", localKitId: "k" }, cron: "0 9 * * *", timezone: "UTC", prompt: "go", budgetCents: 50, approvalId: "appr-x" });
    expect(res.status).toBe(201);
    const s = (await res.json()) as { nextRunAt: string; enabled: boolean; timezone: string };
    expect(s.enabled).toBe(true);
    expect(s.timezone).toBe("UTC");
    // nextRunAt is a valid ISO instant strictly in the future of creation.
    expect(new Date(s.nextRunAt).getTime()).toBeGreaterThan(Date.now() - 1000);
    expect(storageRef.current.state.schedules).toHaveLength(1);
  });
});
