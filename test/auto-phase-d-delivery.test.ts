...
describe("Phase D — deliveryConfig threaded onto a run (cookie create)", () => {
  beforeEach(async () => {
  ...
  it("valid deliveryConfig → 201 and persisted on the run", async () => {
    seedApproval();
  ...
  it("no deliveryConfig → 201 with no delivery on the run (backward compatible)", async () => {
    seedApproval();
    const { POST } = await import("@/app/api/auto/runs/route");
    const res = await POST(
      new Request("https://forge.example/api/auto/runs", {
    ...
    expect(res.status).toBe(201);
    expect((storageRef.current.state.runs[0] as Run).deliveryConfig).toBeUndefined();
  });

  it("http (non-https) webhook url → 400 invalid_request, no run created", async () => {
    seedApproval();
    const { POST } = await import("@/app/api/auto/runs/route");
    const res = await POST(
      new Request("https://forge.example/api/auto/runs", {
        ...
        body: JSON.stringify({
          ...
          input: { prompt: "go" },
          budgetCents: 40,
          deliveryConfig: { webhook: { url: "http://insecure.example.com/x" } }
        })
      })
    );
  ...
  it("malformed email → 400 invalid_request, no run created", async () => {
    seedApproval();
    const { POST } = await import("@/app/api/auto/runs/route");
    const res = await POST(
      new Request("https://forge.example/api/auto/runs", {
        ...
        body: JSON.stringify({
          ...
          input: { prompt: "go" },
          budgetCents: 40,
          deliveryConfig: { email: ["not-an-email"] }
        })
      })
    );
    expect(res.status).toBe(400);
